...
/**
*初始化显示缓冲区
*@param disp\u buf pointer`lv\u disp\u buf\u t`要初始化的变量
*@param buf1 LVGL用来绘制图像的缓冲区。
*始终必须指定，并且不能为空。
*可以是用户分配的数组。例如，`静态lv\u color\u t disp\u buf1[1024*10]`
*或存储器地址，例如在外部SRAM中
*@param buf2（可选）指定第二个缓冲区以进行图像渲染和图像刷新
*（发送到显示器）并行。
*在“disp\u drv->flush”中，您应该使用DMA或类似的硬件发送数据
*图像将显示在背景中。
*它允许LVGL在上一帧被渲染时将下一帧渲染到另一个缓冲区中
*已发送。如果未使用，则设置为“NULL”。
*@param size \u in \u px \u cnt“buf1”和“buf2”的大小（像素计数）。
*/
...
*注册初始化的显示驱动程序。
*自动将第一个显示设置为活动。
*@param driver指向初始化的'lv\u disp\u drv\u t'变量的指针（可以是局部变量）
*@返回指向新显示的指针或出错时为空
*/
...
/**
*在运行时更新驱动程序。
*@param disp指向显示器的指针(返回值'lv\u disp\u drv\u register'）
*@param new\u drv指向新驱动程序的指针
*/

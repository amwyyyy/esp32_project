...
/**
*创建图像对象
// par指向一个对象的指针，它将是新按钮的父级
// copy指向图像对象的指针，如果不为空，则将从中复制新对象
*@return指向已创建图像的指针
*/
...
*如果启用，对象大小将与图片大小相同。
*@param img指向图像的指针
*@param en true:自动大小启用，false:自动大小禁用
*/
...
*图像将围绕此点旋转
*@param img指向图像对象的指针
*@param pivot_x图像旋转中心x
*@param pivot\u y图像旋转中心y
*/
...
/**
*设置图像的旋转角度。
*图像将围绕“lv\u img\u set\u pivot（）设置的轴旋转`
*@param img指向图像对象的指针
*@param-angle旋转角度，以度为单位，分辨率为0.1度（0..3600:时钟方向）
*/
...
*@param img指向图像对象的指针
*@param zoom缩放因子。
*-256或LV\u ZOOM\u IMG\u NONE表示无缩放
*-<256：缩小
*->256放大
*-128半码
*-512双码
 ...
 */
/**
*是否为变换（旋转、缩放）启用/禁用抗锯齿
*@param img指向图像对象的指针
*@param antialas true:抗锯齿；false:未消除混叠
*/
...
*获取自动大小启用属性
*@param img指向图像的指针
*@return true:启用自动调整大小，false:禁用自动调整大小
*/
...
*处理图像的图形相关任务
*@param img指向对象的指针
*@param clip\u区域对象将仅在此区域绘制
*@param mode LV_designment\u COVER\u CHK:仅检查对象是否完全覆盖了“mask\u p”区域
*（如果是，返回“true”）
*LV_UDesign炣绘图：绘制对象（始终返回“true”）
*吕炣设计_u2;画炣帖：每一个孩子画完后绘图
*@param返回'lv_udesign\u res t'元素`
*/
static lv_design_res_t lv_img_design(lv_obj_t * img, const lv_area_t * clip_area, lv_design_mode_t mode)
{
    lv_img_ext_t * ext       = lv_obj_get_ext_attr(img);
...
*图像的信号函数
*@param img指向图像对象的指针
*@param从lv\u signal\u t enum签名信号类型
*@param param指向特定于信号的变量的指针
*@return LV\u RES\u OK:函数中没有删除对象；LV\u RES\u INV:对象被删除
*/
static lv_res_t lv_img_signal(lv_obj_t * img, lv_signal_t sign, void * param)
{
    lv_res_t res;
